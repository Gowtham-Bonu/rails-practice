class Faculty < ApplicationRecord
  ...
  validates :designation, exclusion: { in: %w(HOD Sr.Prof), message: "%{value} is reserved." }
  validates :designation, inclusion: { in: %w(Ass.Prof Prof), message: "%{value} is not a valid designation." }
end
